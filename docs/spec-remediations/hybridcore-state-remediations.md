...
4. **Integrity** – Governance requires diagram/test reporting during remediations, but the state spec does not track which tests produced a given snapshot. _Plan_: store provenance (test suite name, timestamp) alongside each hash.
